...
export default function UpdateDepartment() {
  ...
  useEffect(() => {
    Promise.all([
      authFetch(`${ADMIN_BASE}/departments/${id}`).then(r => r.json()),
      ...
      setDepartments((allDeptRes.data?.content ?? []).filter(d => d.id !== parseInt(id, 10)));
      let d = deptRes.data;
      reset({
        name: d.name,
  ...
  async function onSubmit(data) {
    try {
      ...
      let res = await authFetch(`${ADMIN_BASE}/departments/${id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload),
      });
      if (res.ok) { toast.success('Department updated!'); nav('/admin/departments', { replace: true }); }
  ...
  return (
    <div className="hrms-content">
      <div className="page-header">
        <h4><i className="bi bi-building"></i> Update Department</h4>
      </div>
      <div className="hrms-form-card">
        <form onSubmit={handleSubmit(onSubmit)} noValidate>
          ...
          <div className="d-flex gap-2">
            <button className="btn btn-primary px-4">Update Department</button>
